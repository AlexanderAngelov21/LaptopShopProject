...
public class MainPanel extends JFrame implements ChangeListener{
	...
	JLabel laptopBrand = new JLabel("Въведи марка на лаптопа: ");
	...
	JLabel brandLaptop = new JLabel("Избери марка лаптоп: ");
	JLabel modelLaptop = new JLabel("Въведи модел на лаптоп: ");
	JLabel yearLaptop = new JLabel("Въведи година на производство: ");
	JLabel priceLaptop = new JLabel("Въведи цена на лаптопа: ");
	JLabel commentLaptop = new JLabel("Коментар: ");
	JLabel searchLaptop = new JLabel("Търси по цена(над): ");
	...
	JButton addBtnLaptop = new JButton("Добави");
	JButton deleteBtnLaptop = new JButton("Изтрий");
	JButton editBtnLaptop = new JButton("Промени");
	JButton searchLaptopBtn = new JButton("Търси");
	JButton searchLaptopCancelBtn = new JButton("Отмени търсенето");
	...
	public MainPanel() {
		...
		this.setTitle("Магазин за лаптопи - АлександърТехнолоджис ЕООД");
		...
		tab.add(laptopConfig,"Конфигурация на лаптопи");
		...
		laptopTable.setModel(DBLaptopHelper.getAllData());
		...
		//---------------upPanel Sale CONFIG-------------
		JLabel selectLaptop = new JLabel("Избери лаптоп като натиснеш 1 път върху него: ");
		laptopSelectTable.setModel(DBLaptopHelper.getAllData());
		...
		JLabel laptopCriteriaLabel = new JLabel("          Търсене в таблица с лаптопи:");
		JLabel saleCriteriaLabel = new JLabel("          Търсене в таблица с продажби:");
		
		JLabel brandCriteriaLabel = new JLabel("Въведи марка на лаптопа:");
		JLabel modelCriteriaLabel = new JLabel("Въведи модел на лаптопа:");
		modelCriteriaTF = new JTextField();
		JButton laptopSearchBtn = new JButton("Търси в таблица лаптопи");
		JButton laptopCriteriaCancelBtn = new JButton("Отмяна на търсене");
		//---------------------
		JLabel brandCriteriaLabel2 = new JLabel("Въведи марка на лаптопа:");
		JLabel modelCriteriaLabel2 = new JLabel("Въведи модел на лаптопа:");
		...
		laptopCriteriaTable.setModel(DBLaptopHelper.getAllData());
	...
	class AddAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(currentTab == 0) {
				...
				conn = DBLaptopHelper.getConnection();
				String sql = "insert into LAPTOPS values(DEFAULT,?,?,?,?,?)";
				try {
					state = conn.prepareStatement(sql);
					state.setInt(1, DBLaptopHelper.getBrandData(brandCombo.getSelectedItem().toString())); //brandCombo.getSelectedItem().toString()
		...
		public void actionPerformed(ActionEvent e) {
			if(currentTab == 0) {
				...
				try {
					...
					// TODO Auto-generated catch block
					JOptionPane.showMessageDialog(jframe,"Съществуват лаптопи от следната марка.Ако искате да изтриете марката, изтриите първо всички лаптопи от дадената марка!","Внимание!",JOptionPane.WARNING_MESSAGE);
					//e1.printStackTrace();
				}
				...
				try {
					...
					// TODO Auto-generated catch block
					JOptionPane.showMessageDialog(jframe,"Съществуват записи на продажби на лаптоп със следните параметри, изтриите първо записа на продажбата,за да изтриете лаптопа!","Внимание!",JOptionPane.WARNING_MESSAGE);
					//e1.printStackTrace();
				}
			}
			else if(currentTab == 2) {
	...
	class EditAction implements ActionListener{
		public void actionPerformed(ActionEvent e) {
			if(currentTab == 0) {
				...
				// TODO Auto-generated method stub
				conn = DBBrandHelper.getConnection();
				String sql = "UPDATE LAPTOPS SET BRANDID = \'" + DBLaptopHelper.getBrandData(brandCombo.getSelectedItem().toString()) + "\', MODEL = \'"  + modelLaptopTF.getText() + "\', YEAROFPRODUCTION = \'"  + yearLapopTF.getText() + "\', PRICE = \'"  + priceLaptopTF.getText() + "\', COMMENT = \'"  + commentTF.getText() + "\' WHERE LAPTOPID=?;";
				try {
					state = conn.prepareStatement(sql);
					state.setInt(1, id);
					state.execute();
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(currentTab == 0) {
				...
				try {
					state = conn.prepareStatement(sql);
					state.execute();	
					laptopTable.setModel(DBLaptopHelper.getSearchData(searchLaptopTF.getText()));
		...
		public void actionPerformed(ActionEvent e) {
			// TODO Auto-generated method stub
				...
				try {
					state = conn.prepareStatement(sql);
					state.execute();	
					laptopCriteriaTable.setModel(DBLaptopHelper.getSearchCriteriaData(brandCriteriaCombo.getSelectedItem().toString(),modelCriteriaTF.getText()));
